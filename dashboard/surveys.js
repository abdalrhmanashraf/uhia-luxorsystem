...
function checkAuthSession() {
  var sessionStr = localStorage.getItem('uhia_survey_auth');
  if (sessionStr) {
    try {
      var session = JSON.parse(sessionStr);
      if (session && session.username && session.loggedInAt) {
        var age = Date.now() - session.loggedInAt;
        if (age < 14 * 24 * 60 * 60 * 1000) { // 14 days valid session
          showDashboard(session);
          return true;
        }
      }
    } catch(e) {
      localStorage.removeItem('uhia_survey_auth');
    }
  }
  showLoginScreen();
  return false;
...
function showDashboard(user) {
  ...
  var navUserRoleBadge = document.getElementById('navUserRoleBadge');
  var navUserAvatar = document.getElementById('navUserAvatar');

  if (navUserName) navUserName.innerText = user.name || user.username;
  if (navUserRoleBadge) navUserRoleBadge.innerText = user.badge || user.role || 'إدارة';
...
function doLogin() {
  var now = Date.now();
  if (now < lockoutUntil) {
    var waitSecs = Math.ceil((lockoutUntil - now) / 1000);
    showLoginAlert('تم إيقاف المحاولات مؤقتاً لأسباب أمنية. يرجى الانتظار ' + waitSecs + ' ثانية.', 'error');
    return;
  }
  ...
  var username = uInput ? uInput.value.trim().toLowerCase() : '';
  var password = pInput ? pInput.value.trim() : '';

  if (!username || !password) {
    showLoginAlert('الرجاء إدخال اسم المستخدم وكلمة المرور.', 'warning');
    return;
  }

  if (btn) btn.innerText = '⏳ جارٍ التحقق والتشفير...';

  // Check predefined secure accounts
  if (AUTH_ACCOUNTS[username] && AUTH_ACCOUNTS[username].password === password) {
    var user = AUTH_ACCOUNTS[username];
    var session = {
      username: username,
      name: user.name,
      role: user.role,
    ...
    localStorage.setItem('uhia_survey_auth', JSON.stringify(session));
    loginAttempts = 0;
    if (btn) btn.innerText = '🔐 تسجيل الدخول الآمن';
    showDashboard(session);
    return;
  }

  // Fallback: Check backend API
  if (API_URL) {
    fetch(API_URL, {
      method: 'POST',
      body: JSON.stringify({ action: 'login', phone: username, password: password }),
    ...
    .then(function(res) {
      if (res.success && res.user) {
        var session = {
          ...
          role: res.user.role || 'موظف',
          badge: res.user.role === 'admin' ? '👑 مدير نظام' : '👤 موظف',
          avatar: res.user.role === 'admin' ? '🛡️' : '👤',
          loggedInAt: Date.now()
        };
        localStorage.setItem('uhia_survey_auth', JSON.stringify(session));
        loginAttempts = 0;
...
function handleFailedLogin() {
  ...
  loginAttempts++;
  if (loginAttempts >= 5) {
    lockoutUntil = Date.now() + 15000;
    showLoginAlert('تم استنفاد المحاولات. تم إيقاف تسجيل الدخول لمدة 15 ثانية.', 'error');
  } else {
    showLoginAlert('اسم المستخدم أو كلمة المرور غير صحيحة. يرجى إعادة المحاولة.', 'error');
  }
}
...
function showLoginAlert(msg, type) {
  ...
  if (type === 'error') {
    ...
    alertEl.style.background = 'rgba(245, 158, 11, 0.2)';
